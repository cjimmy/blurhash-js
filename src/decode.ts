...
const validateBlurhash = (blurhash: string) => {
  ...
  const sizeFlag = from83to10(blurhash[0])
...
export const decode = (
  ...
  const quantisedMaximumValue = from83to10(blurhash[1])
  ...
  for (let i = 0; i < colors.length; i++) {
    if (i === 0) {
      const value = from83to10(blurhash.substring(2, 6))
      colors[i] = decodeDC(value)
    } else {
      const value = from83to10(blurhash.substring(4 + i * 2, 6 + i * 2))
